...
int ecg = 0;
float[] ecgHist = new float[dataNum]; 
float[] smoothedECG = new float[dataNum]; //smoothed acc data to show
float[] dEcgHist = new float[dataNum]; 
float[] dSmoothedECGHist = new float[dataNum]; 

int beatECG = 0;
int ECGThld = 20;
boolean flagECG = false;
float[] beatECGHist = new float[dataNum]; //history data to show

float[] IBIHist2   = new float[dataNum];  //history interbeat intervals (IBI)
...
void draw() {
  ...
  if (!bDrawOnly) {
    ...
    lineGraph(smoothedECG, 0, 683, 0, 3*h, width, h, color(255, 0, 0));
    lineGraph(beatECGHist, 0, 1, 0, 3*h, width, h, color(0, 0, 255));
    drawInfo(scale, h);
  }
...
void drawInfo(float scale, float h) {
  ...
  fill(0);
  textAlign(LEFT, CENTER);
  text("ECG and HeartBeat", 0, 3.1*h);
...
void serialEvent(Serial port) {   
  ...
  if (dataIndex==5) {
    ecg = int(trim(inData.substring(1))); //store the value
    appendArray(ecgHist, ecg); //store the data to history (for visualization)
    appendArray(dEcgHist, ecgHist[dataNum-1] - ecgHist[dataNum-2]); //store the data to history (for visualization)
    appendArray(smoothedECG, nextValueECG(ecg));
    float diffS = smoothedECG[dataNum-1] - smoothedECG[dataNum-2]; //absolute diff is used
    appendArray(dSmoothedECGHist, diffS); //store the data to history (for visualization)

    if (diffS>ECGThld) {
      if (!flagECG) {
        flagECG = true;
      }
      appendArray(beatECGHist, 0); //store the data to history (for visualization)
    } else {
      float beat = 0;
      if (flagECG) {
        if (dSmoothedECGHist[dataNum-2]>0 && dSmoothedECGHist[dataNum-1]<=0) {
          beat = 1;
          flagECG = false;
      ...
      appendArray(beatECGHist, beat); //store the data to history (for visualization)
    }
