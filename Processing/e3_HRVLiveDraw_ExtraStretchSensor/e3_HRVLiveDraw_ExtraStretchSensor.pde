...
int breathing = 0;
float[] breathingHist = new float[dataNum];
...
void draw() {
  ...
  if (!bDrawOnly) {
    ...
    lineGraph(breathingHist, 0, 1023, 0, 2*h, width, h, color(0, 0, 255));
    if (bCalibed) {
      lineGraph(breathingHist, minX, maxX, 0, 2*h, width, h, color(0, 128, 255));
    }
    drawInfo(scale, h);
  }
...
void drawInfo(float scale, float h) {
  ...
  textAlign(LEFT, CENTER);
  if (!bCalibed){
    if(calibMode==1) text("Breathing (Raw Data: Need Calibration)", 0, 2.1*h);
    else text("Breathing (Calibrating: Do deep breathing.", 0, 2.1*h);
  }
  else text("Breathing (Calibrated)", 0, 2.1*h);
...
void serialEvent(Serial port) {   
  ...
  if (dataIndex==5) {
    breathing = int(trim(inData.substring(1))); //store the value
    appendArray(breathingHist, breathing); //store the data to history (for visualization)
    if(calibMode==0) testCalib(breathing);
    return;
  }
}
