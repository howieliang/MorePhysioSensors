...
int pulseSensorPin = A5;                 // Pulse Sensor purple wire connected to analog pin 5
int beatPulse = 0;
...
void loop() {
  if (micros() - timer > MICRO_S) { //Timer: send sensor data in every 2ms
    ...
    sendDataToProcessing('C', Signal); //Put the beat into buffer to sent it out later.
    beatPulse = checkHeartBeat(pulseSensorPin);
    sendDataToProcessing('D', beatPulse);
  }
}
