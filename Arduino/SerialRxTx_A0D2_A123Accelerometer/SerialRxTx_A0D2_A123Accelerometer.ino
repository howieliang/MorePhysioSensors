...
void loop() {// put your main code here, to run repeatedly:
  if (micros() - timer > MICRO_S) { //Timer: send sensor data in every 2ms
    ...
    accX = analogRead(A1); //get the analog reading
    sendDataToProcessing('X', accX); //Put the data into buffer to sent it out later.
    accY = analogRead(A2); //get the analog reading
    sendDataToProcessing('Y', accY); //Put the data into buffer to sent it out later.
    accZ = analogRead(A3); //get the analog reading
    sendDataToProcessing('Z', accZ); //Put the data into buffer to sent it out later.   
  }
}
